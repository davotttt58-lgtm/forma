#!/usr/bin/env python3
"""
ФОРМА — AI Персональный Тренер & Диетолог
...
PLANS = {
 ...
 " Базовый план питания", " Умные уведомления", " AI-чат 24/7", "  },
 "pro": {
...
WORKOUTS = {
 ...
 "День 5: Плечи — Жим стоя 3×10, Разводки 3×12, Шраги 3×15",
...
def init_db():
 ...
 for col, defn in [("notify_time", "TEXT DEFAULT NULL"), ("calorie_goal", "INTEGER DEFAULT try:
...
def get_or_create_user(user_id, username, first_name):
 ...
 c.execute("INSERT INTO users (user_id, username, first_name, ref_code) VALUES (?,?,?, (user_id, username, first_name, ref_code))
 conn.commit()
 conn.close()
def get_user(user_id):
 ...
 "ref_code","referred_by","bonus_months","notify_time","calorie_goal","created_at" return dict(zip(cols, row))
def set_user_plan(user_id, plan, months=1):
 conn = sqlite3.connect("forma.db")
 c = conn.cursor()
 expires = (datetime.now() + timedelta(days=30*months)).strftime("%Y-%m-%d")
 c.execute("UPDATE users SET plan=?, plan_expires=? WHERE user_id=?", (plan, expires, user conn.commit()
 conn.close()
...
def save_referral(referrer_id, referee_id):
 ...
 c.execute("INSERT INTO referrals (referrer_id, referee_id) VALUES (?,?)", (referrer_id, r conn.commit()
 conn.close()
...
def get_ai_history(user_id, limit=10):
 conn = sqlite3.connect("forma.db")
 c = conn.cursor()
 c.execute("SELECT role, content FROM ai_chat_history WHERE user_id=? ORDER BY id DESC LIM (user_id, limit))
 rows = c.fetchall()
 conn.close()
...
def save_ai_message(user_id, role, content):
 ...
 (SELECT id FROM ai_chat_history WHERE user_id=? ORDER BY id DESC LIMIT 20)"" (user_id, user_id))
 conn.commit()
 conn.close()
...
def save_food_entry(user_id, food_name, calories, protein, fat, carbs):
 conn = sqlite3.connect("forma.db")
 c = conn.cursor()
 c.execute("INSERT INTO food_diary (user_id, food_name, calories, protein, fat, carbs) VAL (user_id, food_name, int(calories), float(protein), float(fat), float(carbs)))
 conn.commit()
 conn.close()
def get_today_food(user_id):
 conn = sqlite3.connect("forma.db")
 c = conn.cursor()
 c.execute("SELECT food_name, calories, protein, fat, carbs FROM food_diary WHERE user_id= (user_id,))
 rows = c.fetchall()
 conn.close()
 return rows
def get_stats():
 ...
 c.execute("SELECT COUNT(*) FROM users WHERE plan NOT IN ('free','start')"); paid_users =  c.execute("SELECT SUM(amount) FROM payments WHERE status='success'"); revenue = c.fetchon c.execute("SELECT COUNT(*) FROM referrals"); total_refs = c.fetchone()[0]
 c.execute("SELECT COUNT(*) FROM users WHERE created_at >= date('now', '-7 days')"); new_w c.execute("SELECT COUNT(*) FROM ai_chat_history"); ai_msgs = c.fetchone()[0]
 ...
 total_refs=total_refs, new_week=new_week, ai_msgs=ai_msgs, food_entries=food_def get_top_referrers():
 conn = sqlite3.connect("forma.db")
 c = conn.cursor()
...
def get_all_users_with_notify():
 conn = sqlite3.connect("forma.db")
 c = conn.cursor()
 c.execute("SELECT user_id, first_name, notify_time, goal FROM users WHERE notify_time IS  rows = c.fetchall()
 conn.close()
 return rows
# ══════════════════════════════════════════════════════════════
# CLAUDE API
...
async def call_claude(messages: list, system: str = "", max_tokens: int = 800) -> str:
 ...
 return " AI-тренер временно недоступен. Администратор должен добавить ANTHROPIC_API try:
 async with httpx.AsyncClient(timeout=30) as client:
 payload = {"model": "claude-sonnet-4-20250514", "max_tokens": max_tokens, "messag if system:
 ...
 return " Не удалось получить ответ от AI. Попробуй чуть позже."
async def call_claude_vision(image_bytes: bytes, prompt: str) -> str:
 if not ANTHROPIC_API_KEY:
 return " AI-анализ недоступен. Нужен ANTHROPIC_API_KEY."
 try:
 ...
 {"type": "image", "source": {"type": "base64", "media_type": "image/j {"type": "text", "text": prompt},
 ]}]
 }
 )
 data = resp.json()
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
 ...
 keyboard.append([InlineKeyboardButton(" Админ-панель", callback_data="admin_menu")] await update.message.reply_text(
...
async def ai_chat_info(update: Update, context: ContextTypes.DEFAULT_TYPE):
 ...
 await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mo
 return MAIN_MENU
async def ai_clear_history(update: Update, context: ContextTypes.DEFAULT_TYPE):
 ...
 reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("← Назад", callback_data="ai )
 return MAIN_MENU
...
async def notify_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
 ...
 labels = {"morning": " Утро (8:00)", "afternoon": " День (13:00)", "evening": " Ве current_text = labels.get(current, "не настроены")
 text = (
 ...
 keyboard = [
 [InlineKeyboardButton(" Утро (8:00)", callback_data="notify_morning")],
 ...
 await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mo return MAIN_MENU
async def set_notify(update: Update, context: ContextTypes.DEFAULT_TYPE):
 query = update.callback_query
 await query.answer()
 action = query.data
 time_map = {
 "notify_morning": ("morning", " Утро (8:00)"),
...
async def send_morning_reminders(app):
 ...
 InlineKeyboardButton(" Тренировка выполнена!", callback_data="workout_d ]]),
 parse_mode="Markdown"
 )
 except Exception:
 pass
...
async def send_late_reminders(app):
 ...
 reply_markup=InlineKeyboardMarkup([
 [InlineKeyboardButton(" Сделал тренировку!", callback_data="workout_don [InlineKeyboardButton(" Пропустить сегодня", callback_data="back_main") ]),
 parse_mode="Markdown"
 )
 except Exception:
 pass
...
async def food_diary_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
 ...
 await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard), pars return MAIN_MENU
...
async def handle_food_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
 ...
 prompt = (
 "Проанализируй блюдо на фото. Ответь СТРОГО в этом формате (без лишнего текста):\ "БЛЮДО: [название на русском]\n"
 "КАЛОРИИ: [число]\n"
 "БЕЛКИ: [число]г\n"
 "ЖИРЫ: [число]г\n"
 "УГЛЕВОДЫ: [число]г\n"
 ...
 if "НЕ_ЕДА" in result:
 await msg.edit_text(" Не вижу еду на фото. Попробуй сфотографировать блюдо круп return
 # Парсинг ответа
 food_data = {}
 ...
 await msg.edit_text(response_text, reply_markup=InlineKeyboardMarkup(keyboard), parse except Exception as e:
...
async def free_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
 query = update.callback_query
 await query.answer()
 text = " *Выбери свою главную цель:*\n\nОт этого зависит весь план тренировок и питания keyboard = [
 [InlineKeyboardButton(" Похудеть", callback_data="goal_похудение")],
 [InlineKeyboardButton(" Набрать мышечную массу", callback_data="goal_набор массы")] [InlineKeyboardButton(" Поддержать форму", callback_data="goal_поддержание")],
 [InlineKeyboardButton("← Назад", callback_data="back_main")],
 ]
 await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mo return CHOOSE_GOAL
async def choose_goal(update: Update, context: ContextTypes.DEFAULT_TYPE):
 ...
 [InlineKeyboardButton(" Средний (6 мес – 2 года)", callback_data="level_intermediat [InlineKeyboardButton(" Продвинутый (2+ года)", callback_data="level_advanced")],
...
async def choose_level(update: Update, context: ContextTypes.DEFAULT_TYPE):
 ...
 level_names = {"beginner": "Новичок", "intermediate": "Средний", "advanced": "Продвинутый keyboard = [
 [InlineKeyboardButton(" Дома (без оборудования)", callback_data="place_home")],
 [InlineKeyboardButton(" В зале", callback_data="place_gym")],
...
async def show_workout_plan(update: Update, context: ContextTypes.DEFAULT_TYPE):
 ...
 workouts = WORKOUTS.get(goal, WORKOUTS["поддержание"]).get(level, WORKOUTS["поддержание"] place_emoji = {"home": " ", "gym": " ", "street": " "}.get(place, " ")
 ...
 await query.edit_message_text(plan_text, reply_markup=InlineKeyboardMarkup(keyboard), par return SHOW_PLAN
async def nutrition_plan(update: Update, context: ContextTypes.DEFAULT_TYPE):
 ...
 await query.edit_message_text(plan, reply_markup=InlineKeyboardMarkup(keyboard), parse_mo
...
async def show_plans(update: Update, context: ContextTypes.DEFAULT_TYPE):
 ...
 await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mo return SUBSCRIPTION_MENU
async def plan_detail(update: Update, context: ContextTypes.DEFAULT_TYPE):
 ...
 text = f"*{plan['name']}*\n {price_text}\n\n{features_text}\n\n━━━━━━━━━━━━━━━━━━\nВыбе keyboard = []
 if plan["price"] == 0:
 keyboard.append([InlineKeyboardButton(" Активировать бесплатно", callback_data="act else:
 keyboard.append([InlineKeyboardButton(" Оплатить через ЮКассу", callback_data=f"pay keyboard.append([InlineKeyboardButton("← К тарифам", callback_data="show_plans")])
 await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mo return SUBSCRIPTION_MENU
async def activate_free(update: Update, context: ContextTypes.DEFAULT_TYPE):
 ...
 keyboard = [
 [InlineKeyboardButton(" Получить план", callback_data="free_start")],
 [InlineKeyboardButton(" Настроить уведомления", callback_data="notify_menu")],
 [InlineKeyboardButton(" Главное меню", callback_data="back_main")],
 ]
 await query.edit_message_text(
 " *Тариф Старт активирован!*\n\nУ тебя есть *7 дней бесплатного доступа*\n\nС чего  reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown"
 )
 return MAIN_MENU
async def process_payment(update: Update, context: ContextTypes.DEFAULT_TYPE):
 ...
 await query.edit_message_text(
 " Платёжная система не настроена.\nДобавьте YOOKASSA_TOKEN в переменные Railway reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("← Назад", callback_data )
 ...
 "receipt": {
 "tax_system_code": 1, # ОСН (1), УСН доход (2), УСН доход-расход (3), ЕН "items": [
 {
 ...
 await context.bot.send_message(
 chat_id = query.from_user.id,
 text = f" Ошибка создания счёта: {str(e)[:200]}\n\nНапиши @FormaSupport"
 )
 return SUBSCRIPTION_MENU
...
async def successful_payment(update: Update, context: ContextTypes.DEFAULT_TYPE):
 ...
 c.execute("UPDATE referrals SET bonus_given=1 WHERE referrer_id=? AND referee_id= (referrer_id, user.id))
 conn.commit()
 conn.close()
 add_bonus_month(referrer_id)
 try:
 await context.bot.send_message(referrer_id,
 " Твой реферал оплатил подписку!\n Тебе начислен *+1 месяц* бесплатно parse_mode="Markdown")
 except Exception:
 pass
 else:
 conn.close()
 plan = PLANS.get(plan_key, PLANS["forma"])
 extra = "\n\n AI-тренер и фото-анализ еды теперь доступны!" if plan_key in ("pro","r
 await update.message.reply_text(
...
async def referral_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
 ...
 " Друг регистрируется по ней\n"
 " *Тебе* — +1 месяц бесплатно \n"
 " *Другу* — скидка 20% на первую оплату\n\n"
 "━━━━━━━━━━━━━━━━━━\n"
 ...
 keyboard = [
 [InlineKeyboardButton(" Поделиться", switch_inline_query=f"Присоединяйся к ФОРМА! { [InlineKeyboardButton("← Назад", callback_data="back_main")],
 ]
 await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mo return MAIN_MENU
async def my_profile(update: Update, context: ContextTypes.DEFAULT_TYPE):
 ...
 bot_username = (await context.bot.get_me()).username
 ref_link = f"https://t.me/{bot_username}?start={db_user['ref_code']}"
 plan_names = {"free":" Старт","start":" Старт","forma":" Форма","pro":" Прокач notify_names = {"morning":" 8:00","afternoon":" 13:00","evening":" 19:00",None:"не  today_food = get_today_food(user.id)
 ...
 await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mo return PROFILE_MENU
async def about(update: Update, context: ContextTypes.DEFAULT_TYPE):
 ...
 " *Что умею:*\n"
 "• Планы тренировок под любую цель\n"
 "• Планы питания с калорийностью\n"
...
async def back_main(update: Update, context: ContextTypes.DEFAULT_TYPE):
 ...
 if user.id in ADMIN_IDS:
 keyboard.append([InlineKeyboardButton(" Админ-панель", callback_data="admin_menu")] await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mo return MAIN_MENU
# ══════════════════════════════════════════════════════════════
# АДМИН-ПАНЕЛЬ
# ══════════════════════════════════════════════════════════════
async def admin_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
 ...
 await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mo return ADMIN_MENU
async def admin_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
 ...
 c.execute("SELECT SUM(amount), COUNT(*) FROM payments WHERE status='success'"); pay_row = c.execute("SELECT COUNT(*) FROM users WHERE created_at >= date('now', '-1 day')"); new_to conn.close()
...
async def admin_referrers(update: Update, context: ContextTypes.DEFAULT_TYPE):
 ...
 lines = "".join(f"{i}. {'@'+u if u else n} — {cnt}\n" for i,(n,u,cnt) in enumerate(top,1) text = f" *Топ рефереры*\n\n━━━━━━━━━━━━━━━━━━\n{lines}━━━━━━━━━━━━━━━━━━"
 keyboard = [[InlineKeyboardButton("← Назад", callback_data="admin_menu")]]
 await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mo return ADMIN_MENU
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
 ...
 await thinking.edit_text(response, reply_markup=InlineKeyboardMarkup(keyboard), parse return
 # ── Статические ответы для бесплатных ────────────────────
 text_lower = text.lower()
 static = {
 "белок": " *Белок* — строительный материал для мышц.\nНорма: *1.6–2.2г на кг веса* "калори": " *Калорийность* зависит от цели:\n• Похудение: дефицит 300–500 ккал\n• Н "вода": " *Норма воды:* 30–40 мл на кг веса.\nПри тренировках +500–700 мл дополни "отжим": " *Техника отжиманий:*\n1. Руки чуть шире плеч\n2. Тело — прямая линия\n3 }
 ...
 await update.message.reply_text(reply, reply_markup=InlineKeyboardMarkup(keyboard), parse# ══════════════════════════════════════════════════════════════
# MAIN
# ══════════════════════════════════════════════════════════════
def main():
 init_db()
 ...
 CallbackQueryHandler(set_notify, pattern="^notify_(morning|afternoon|evening|o CallbackQueryHandler(mark_workout_done, pattern="^workout_done$"),
 ...
 states={
 MAIN_MENU: cbs,
 CHOOSE_GOAL: cbs + [CallbackQueryHandler(choose_goal, pattern="^goal CHOOSE_LEVEL: cbs + [CallbackQueryHandler(choose_level, pattern="^leve CHOOSE_PLACE: cbs + [CallbackQueryHandler(show_workout_plan, pattern="^plac SHOW_PLAN: cbs,
 NUTRITION_MENU: cbs,
 ...
 print(" ФОРМА Bot запущен! AI-чат + умные пуши + фото-анализ еды активны.")
